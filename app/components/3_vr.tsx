...
function SceneController({ sceneState }: { sceneState: number }) {
    ...
    // 状態 1 (晴れ): 強さ 2 / 状態 2 (霧): 強さ 0.3
    const lightIntensity = isFoggy ? 0.3 : 2; 

    // 霧の制御ロジック
    useFrame(({ scene }) => {
        if (isFoggy) {
            // 💡 状態 2: 暗く濃い霧 (色: 灰色、Near 5, Far 50)
            scene.fog = new THREE.Fog(0x909090, 5, 50); 
        } else {
            // 状態 1: 霧を削除（今まで通り）
            scene.fog = null;
        }
    ...
    return (
        <>
            ...
            <directionalLight 
                position={[5, 10, 5]} 
                intensity={lightIntensity} // 状態によって明るさを変更
                castShadow            
...
export default function ThreeVR({onComplete}:StageProps) {
    ...
    return (
            <Canvas 
                ...
                style={{ backgroundColor: sceneState === 2 ? '#a0aa90' : '#d0e0ff' }}
            >
                <AnimatedSky />
                
                <Clouds material={THREE.MeshLambertMaterial} limit={200}> 
                    <RandomClouds count={100} radius={80} height={15} /> 
                </Clouds>
